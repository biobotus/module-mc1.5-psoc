...
/*******************************************************************************
* Function Name: ClockTurbido_Start
********************************************************************************
*
* Summary:
...
void ClockTurbido_Start(void) 
{
    ...
    ClockTurbido_CLKEN |= ClockTurbido_CLKEN_MASK;
	ClockTurbido_CLKSTBY |= ClockTurbido_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ClockTurbido_Stop
********************************************************************************
*
* Summary:
...
void ClockTurbido_Stop(void) 
{
    ...
    ClockTurbido_CLKEN &= (uint8)(~ClockTurbido_CLKEN_MASK);
	ClockTurbido_CLKSTBY &= (uint8)(~ClockTurbido_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ClockTurbido_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ClockTurbido_CLKEN & ClockTurbido_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ClockTurbido_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ClockTurbido_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ClockTurbido__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ClockTurbido_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ClockTurbido_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ClockTurbido_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ClockTurbido_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ClockTurbido_GetSourceRegister();
    uint16 oldDivider = ClockTurbido_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ClockTurbido_CLKEN & ClockTurbido_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ClockTurbido_DIV_PTR, clkDivider);
                ClockTurbido_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ClockTurbido_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ClockTurbido_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ClockTurbido_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ClockTurbido_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ClockTurbido_DIV_PTR);
}


/*******************************************************************************
* Function Name: ClockTurbido_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockTurbido_MOD_SRC |= modeBitMask & (uint8)ClockTurbido_MODE_MASK;
}


/*******************************************************************************
* Function Name: ClockTurbido_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ClockTurbido_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ClockTurbido_MODE_MASK));
}


/*******************************************************************************
* Function Name: ClockTurbido_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ClockTurbido_MOD_SRC & (uint8)(ClockTurbido_MODE_MASK);
}


/*******************************************************************************
* Function Name: ClockTurbido_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ClockTurbido_GetDividerRegister();
    uint8 oldSrc = ClockTurbido_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ClockTurbido_MOD_SRC |= CYCLK_SSS;
        ClockTurbido_MOD_SRC =
            (ClockTurbido_MOD_SRC & (uint8)(~ClockTurbido_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ClockTurbido_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ClockTurbido_MOD_SRC & ClockTurbido_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ClockTurbido_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ClockTurbido_PHASE = clkPhase & ClockTurbido_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ClockTurbido_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ClockTurbido_PHASE & ClockTurbido_PHASE_MASK;
}
