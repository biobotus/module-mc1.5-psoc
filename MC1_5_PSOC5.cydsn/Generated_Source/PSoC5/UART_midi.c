...
***************************************/
    ...
    volatile uint8 UART_midi_in_ep;                                    /* Input endpoint number */
...
/*******************************************************************************
* Function Name: UART_MIDI_EP_Init
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  UART_midiInBuffer: This buffer is used for saving and combining
...
*  UART_midiOutBuffer: This buffer is used by the
*    UART_MIDI_OUT_EP_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*    UART_callbackLocalMidiEvent function.
*  UART_midi_out_ep: Used as an OUT endpoint number.
*  UART_midi_in_ep: Used as an IN endpoint number.
*   UART_midiInPointer: Initialized to zero.
*
* Reentrant:
*  No
*
...
void UART_MIDI_EP_Init(void) 
{
    ...
    /*******************************************************************************
    * Function Name: UART_MIDI_OUT_EP_Service
    ********************************************************************************
    *
    * Summary:
    ...
    *  UART_callbackLocalMidiEvent() function to internal process
    *  of the MIDI data.
    ...
    *
    * Global variables:
    *  UART_midiOutBuffer: Used as temporary buffer between USB internal
    ...
    *******************************************************************************/
    void UART_MIDI_OUT_EP_Service(void) 
    {
        ...
        if (UART_GetEPState(UART_midi_out_ep) == UART_OUT_BUFFER_FULL)
        {
            #if(UART_MIDI_OUT_BUFF_SIZE >= 256)
                ...
                outLength = (uint8)UART_GetEPCount(UART_midi_out_ep);
                    ...
                    {
                        (void) CyDmaChStatus(UART_DmaChan[UART_midi_out_ep], NULL, &dmaState);
                    }
            ...
            if (outLength >= UART_EVENT_LENGTH)
            {
                ...
                while (outPointer < outLength)
                {
                    ...
                    UART_callbackLocalMidiEvent(
                        UART_midiOutBuffer[outPointer] & UART_CABLE_MASK,
...
#endif /* #if (UART_MIDI_OUT_BUFF_SIZE > 0) */
    ...
    *******************************************************************************/
    void UART_MIDI_IN_EP_Service(void) 
    {
        ...
        if (UART_midiInPointer != 0u)
        {
            ...
            #else /* UART_EP_MM != UART__EP_DMAAUTO */
                /* rearm IN EP */
        ...
        */
            ...
            do
            {
                if (UART_midiInPointer <=
                    (UART_MIDI_IN_BUFF_SIZE - UART_EVENT_LENGTH))
                {
    ...
    * Return:
    *  UART_TRUE if error.
    *  UART_FALSE if success.
    *
    * Global variables:
    *  UART_midi_in_ep: MIDI IN endpoint number used for sending data.
    *  UART_midiInPointer: Checked this variable to see if there is
...
#endif /* #if (UART_MIDI_IN_BUFF_SIZE > 0) */
