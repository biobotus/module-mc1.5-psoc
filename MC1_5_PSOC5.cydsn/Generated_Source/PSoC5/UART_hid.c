...
volatile uint8 UART_hidProtocol[UART_MAX_INTERFACES_NUMBER];  /* HID device protocol status */
volatile uint8 UART_hidIdleRate[UART_MAX_INTERFACES_NUMBER];  /* HID device idle reload value */
    ...
    {   /* Control Read */
        switch (CY_GET_REG8(UART_bRequest))
        {
            case UART_GET_DESCRIPTOR:
                if (CY_GET_REG8(UART_wValueHi) == UART_DESCR_HID_CLASS)
                {
                ...
                else if (CY_GET_REG8(UART_wValueHi) == UART_DESCR_HID_REPORT)
                {
            ...
            case UART_HID_GET_IDLE:
                ...
                if( (interfaceNumber < UART_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(UART_wValueLo) == 0u ) ) /* Do not support Idle per Report ID */
                {
        ...
        switch (CY_GET_REG8(UART_bRequest))
        {
            ...
            case UART_HID_SET_IDLE:
                ...
                if( (interfaceNumber < UART_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(UART_wValueLo) == 0u ) ) /* Do not support Idle per Report ID */
                {
                    UART_hidIdleRate[interfaceNumber] = CY_GET_REG8(UART_wValueHi);
            ...
            case UART_HID_SET_PROTOCOL:
                ...
                if( (interfaceNumber < UART_MAX_INTERFACES_NUMBER) &&
                    (CY_GET_REG8(UART_wValueLo) <= 1u) )
                {
                    UART_hidProtocol[interfaceNumber] = CY_GET_REG8(UART_wValueLo);
                    requestHandled = UART_InitNoDataControlTransfer();
                }
                break;
            default:    /* requestHandled is initialized as FALSE by default */
...
void UART_FindHidClassDecriptor(void) 
{
    ...
    interfaceN = CY_GET_REG8(UART_wIndexLo);
...
void UART_FindReportDescriptor(void) 
{
    ...
    UART_currentTD.count =  (((uint16)pDescr[1u] << 8u) | pDescr[0u]);
...
void UART_FindReport(void) 
{
    ...
    pTmp = UART_GetConfigTablePtr(UART_configuration - 1u);
    reportType = CY_GET_REG8(UART_wValueHi);
    interfaceN = CY_GET_REG8(UART_wIndexLo);
    ...
    if(interfaceN < UART_MAX_INTERFACES_NUMBER)
    {
           ...
           (reportType <= UART_HID_GET_REPORT_FEATURE))
        {
            ...
            reportType = CY_GET_REG8(UART_wValueLo);    /* Get reportID */
